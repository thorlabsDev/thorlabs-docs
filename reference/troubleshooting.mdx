...
| Quota-style monthly metadata appears        | Read the [monthly and live limit behavior](/reference/plans-and-limits#understand-the-monthly-and-live-limits).      | ThorEdge RPC continues to use live throughput and concurrency limits.                      | Troubleshoot the actual status code or live limit instead of the metadata field name.                  | Depends on the actual failure.           |
...
| Yellowstone cannot connect or rejects a subscription             | Client/protobuf version, plaintext HTTP/2 (`h2c`) mode, filters, and **Limits → gRPC Yellowstone**.         | One named filter connects and returns an update.                                         | Use the pinned proto, correct the request, reduce filters, and close unused streams.                                                 | **No** for request errors; **yes** with backoff after transient disconnects.   |
| Pulse handshake or authentication fails                          | Target, ALPN `pulse`, wire version `2`, control-message token, and trusted network or UDP-capable VPN path. | The first control message returns an `ok: true` acknowledgement.                         | Correct the network path, target, wire version, or token, then reconnect.                                                            | **No**, until corrected.                                                       |
...
See [Yellowstone gRPC](/products/yellowstone-grpc) and [Pulse Decoded Shreds](/products/pulse) for their connection settings.
