...
| Bifrost Transaction Routes | The dashboard issues one Private TX key per access for its enabled Bifrost locations | Use the complete route URL copied for the selected location and operation only after HTTPS and key rejection checks pass.       |
...
- For Bifrost, continue only when the complete route URL copied from **Private TX** begins with `https://` and the route rejects a missing or invalid Private TX key. ThorNode manages the upstream provider credential; do not add or expose a provider API key.
