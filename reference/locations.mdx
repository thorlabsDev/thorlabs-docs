...
| Bifrost Transaction Routes | The dashboard reuses one Bifrost key for the selected access across its enabled Bifrost location URLs. Copy each provider route URL from the intended location; ThorNode manages the upstream provider credentials. |
...
2. Prepare the destination according to its scope:
   ...
   - For Bifrost, select the destination in **Bifrost** and copy the provider route URL shown there. The access key remains the same, but the location URL changes.
