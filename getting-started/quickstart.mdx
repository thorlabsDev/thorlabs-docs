...
After the request works from a server with a stable egress address, enter its real, globally routable public IPv4 value in **Restrict to IP — _location_ (optional)** and choose **Save restriction**. The field rejects common private, local, and multicast ranges, but field acceptance alone does not prove routability. The same restriction applies to RPC and WebSocket traffic for that location token.
