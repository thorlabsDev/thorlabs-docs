...
Use the ThorNode dashboard to turn an owned pass or active rental into an RPC credential or another explicitly supported product flow, then verify that the access remains operational.

Open it from [thornode.io](https://www.thornode.io) and choose **Login with Discord**. The authenticated dashboard is the source of truth for your current access and dynamic product values.
...
| Create an RPC or WebSocket credential | **Endpoints → Endpoints → access → Network → Location** | **Token & Endpoints** shows a masked token and connection details                         |
...
Static documentation defines the workflow and credential semantics. It does not override the values returned for your authenticated account.
