...
Register a Solana wallet when ThorNode needs to discover a pass held by that address or verify the wallet used for a rental or renewal payment. Registration succeeds when the address appears under **Verified wallets**.
...
## Use a registered wallet for payment

The wallet connected during checkout must match an address under **Verified wallets**. The dashboard shortens addresses in the list; use the row's copy action when you need the complete value.
...
3. Confirm that the connected wallet has the required asset and enough balance.
