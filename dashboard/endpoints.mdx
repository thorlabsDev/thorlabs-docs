...
Use **Endpoints → Endpoints** to create RPC and WebSocket credentials for one access, network, and location. You are finished when **Token & Endpoints** shows the selected location's token and connection details.
...
<Steps>
  ...
  <Step title="Generate the token">
    ...
    Generation succeeds when the panel shows a masked region-bound token plus the **RPC** and **WS** rows. If streaming targets are configured for that location, they appear in the **Streaming** section.
...
<Steps>
  ...
  <Step title="Save the restriction">
    ...
    The field accepts one IPv4 value and rejects common private, local, and
    multicast ranges. Enter the deployment's real, globally routable public
    egress address; syntactic acceptance alone does not prove that the value is
    routable. The field does not accept a hostname, URL, port, CIDR range, IPv6
    address, or address list.
