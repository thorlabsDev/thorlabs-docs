...
Use **Usage** to verify what your application sent and **Limits** to read the published tier row resolved for the selected access. Live throughput and concurrency limits still apply; use the [canonical limit model](/reference/plans-and-limits#understand-the-monthly-and-live-limits) for monthly semantics.
...
<Steps>
  ...
  <Step title="Confirm the request was recorded">
    ...
    After a test request, success is observable when the request count or method activity increases for the matching access, location, and range. Usage telemetry can arrive after the response, so use the application's status code for immediate debugging.
...
| Section             | What it tells you                                                            | Success signal                                                                   |
| ------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **Live throughput** | Current request and transaction rate for locations with tokens               | The intended location reports a live connection and current activity             |
| **Plan limits**     | Published RPC, WebSocket, and supported streaming caps for the resolved tier | The selected access resolves to a tier row; legacy access can use a fallback row |
| **Consumption**     | Calls observed in the current reporting period                               | The count reflects recorded usage; it is not a monthly cutoff                    |
| **Calls by token**  | How observed calls are distributed across tokens                             | The deployment's token appears with the expected share of activity               |
...
| Several products fail at once          | **Overview → Network status** or **Node Status**, when available          | No published incident explains the time window                    | If healthy, isolate the failing access and contact support with redacted details |
