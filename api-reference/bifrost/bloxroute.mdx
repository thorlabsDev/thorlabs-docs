...
| `POST /blxrt-submit-batch` | Batch | 1 or more | `useBundle: true`: last transaction; `false` or omitted: every transaction |
...
With `useBundle: true`, the last transaction must pay the qualifying 1,000,000-lamport bloXroute tip. With `useBundle: false` or when `useBundle` is omitted, every transaction is independent and must pay its own qualifying tip.
