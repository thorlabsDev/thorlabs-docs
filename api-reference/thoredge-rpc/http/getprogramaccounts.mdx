...
Answered by [FastGate](/api-reference/fastgate/overview): `commitment` and `minContextSlot` are accepted and ignored, the response reflects FastGate's indexed slot (`context.slot`, no `apiVersion`), and the request times out after 30 s. `filters` are optional. Accepts `changedSinceSlot` in addition to the standard options. At most 10,000 accounts; a larger result returns `-32600` and asks you to use [`getProgramAccountsV2`](/api-reference/fastgate/getprogramaccountsv2). Standard `dataSize` and `memcmp` filters apply; `memcmp.bytes` is base-58 unless `encoding` is `"base64"`.
