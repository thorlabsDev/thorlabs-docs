...
Answered by [FastGate](/api-reference/fastgate/overview): `commitment` and `minContextSlot` are accepted and ignored, the response reflects FastGate's indexed slot (`context.slot`, no `apiVersion`), and the request times out after 30 s. The second parameter (`{ "mint" }` or `{ "programId" }`) is required and must contain exactly one key. For very large owners use [`getTokenAccountsByOwnerV2`](/api-reference/fastgate/gettokenaccountsbyownerv2).
