...
| Pass tier | Fast lanes (`/hofund`, `/hofund-fast`, `/hofund-direct-tpu`, `/hofund-swqos`, `/hofund/plain`, `/hofund/binary`, `/hofund/ws`) | Jito lanes (`?mev-protect=true`, `/hofund-jito-bundle-only`, `/hofund-bundle`, `/hofund/ws/mev-protect`) |
| --- | --- | --- |
...
- One key has **one** fast-lane bucket shared by every fast surface — HTTP and WebSocket draw from the same allowance.
...
| WebSocket | **No reply** — the frame over the rate is dropped silently. Pace your sends under your tier and reconcile every signature. |
