...
The current production endpoint is `rpc-fra1.thornode.io:4802`. The lane is fixed by the port; nothing on the wire selects it. The dashboard lists **Endpoints → Hofund → QUIC ingress** only when that account already has its usable Bifrost key. Use the endpoint shown there; a location that is not listed does not expose customer QUIC ingress.
