...
<Note>
  `sendIdeal` and `getNonce` require both the fast and the Jito lane; both are
  live, but ThorNode has not yet run this pair end to end in production. Treat
  this page as the interface contract and expect it to be verified shortly.
</Note>
