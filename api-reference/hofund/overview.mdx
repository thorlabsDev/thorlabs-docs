...
Ordinary `sendTransaction` routes, including `/hofund-fast`, require a recent blockhash and reject durable nonce. Durable nonce is supported only by the verified [`/hofund` `sendIdeal` pair flow](/api-reference/hofund/sendideal), when that capability is available.
