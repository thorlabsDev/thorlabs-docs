...
Every single-transaction route, including `/hofund-fast`, supports recent blockhash and caller-owned durable nonce transactions. The same support applies to each independent `sendBatch` member and to WebSocket/QUIC ingestion. See [nonce requirements](/api-reference/hofund/sendtransaction#durable-nonce). [`sendIdeal`](/api-reference/hofund/sendideal) additionally verifies and submits a fast/Jito pair sharing one live nonce. Atomic `sendBundle` requests still require recent blockhash transactions.
...
- Every transaction is canonical, padded, standard base64 of a fully signed transaction, at most **1232 bytes** decoded. Single sends and independent batch members accept structurally valid durable nonces; `sendIdeal` additionally verifies its pair against the live nonce account. Duplicate signatures are rejected.
