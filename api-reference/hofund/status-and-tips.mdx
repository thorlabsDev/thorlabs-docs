...
An accepted durable nonce submission with an unobserved signature has status `unknown`. `expiresAt` controls receipt retention and does not establish nonce expiry.

The record lives about 10 minutes. After that, or for WebSocket and QUIC submissions (which return no submission id), reconcile the signature through [ThorEdge RPC](/api-reference/thoredge-rpc/http-methods).
