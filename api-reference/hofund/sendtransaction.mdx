...
Every ordinary `sendTransaction` route requires a recent blockhash. Durable-nonce transactions are rejected; when the capability is available, use the two-transaction durable-nonce pair flow on [`/hofund` `sendIdeal`](/api-reference/hofund/sendideal).
...
| `400` | `-32602` | `durable nonce is not supported by this route; use a recent blockhash, or use /hofund sendIdeal when available` | Rebuild the ordinary transaction with a recent blockhash, or use the supported `sendIdeal` pair flow. |
