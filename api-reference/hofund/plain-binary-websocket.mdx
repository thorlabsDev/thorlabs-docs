...
These surfaces take the same fast and protected lanes as `/hofund` for clients that would rather not build JSON. Plain and binary HTTP accept `sendTransaction`, `sendBatch`, and `getHealth`; the `sendBundle`, `sendIdeal`, and `getNonce` methods require JSON-RPC.

Plain, binary, and WebSocket submissions support recent blockhash and caller-owned durable nonce transactions. Use the same [nonce requirements](/api-reference/hofund/sendtransaction#durable-nonce) and send the signed bytes unchanged.
...
- Frames over your tier's rate are dropped without notice — see [Rate limits](/api-reference/hofund/rate-limits) and pace your sends.
