...
Every bundle member must use a recent blockhash. Durable nonce transactions are supported by [single sends](/api-reference/hofund/sendtransaction#durable-nonce) and [independent batches](/api-reference/hofund/sendbatch), but are rejected by both atomic bundle routes below.
