...
| Bifrost | Provider routes on a Bifrost URL | [Routes](/api-reference/bifrost/overview) |
...
Bifrost and Hofund use a separate Bifrost key in the same path position on a different base URL. Yellowstone sends the location token as `x-token` gRPC metadata. Each reference page states its own placement.
