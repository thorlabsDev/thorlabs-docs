...
You need an active access with ShredStream enabled, Node.js, and a receiver reachable at a numeric, globally routable public IPv4 address. Open the chosen UDP port from `1024` through `65535` in every firewall, security group, network ACL, and NAT rule on the path. Do not use a hostname, private address, loopback address, or IPv6 address.
...
<Steps>
  ...
  <Step title="Register the public target">
    ...
    address and the same UDP port. Save the registration and watch the receiver
    for packets.
  </Step>
...
Traffic is reaching the application when the packet count is nonzero. The listening message only means the local socket opened.
