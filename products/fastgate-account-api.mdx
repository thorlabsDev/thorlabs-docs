...
Follow the [FastGate quickstart](/products/fastgate-account-api/quickstart) to read one account and page through a program with `getProgramAccountsV2`.
...
Standard methods keep the shapes in the [Solana JSON-RPC reference](https://solana.com/docs/rpc). The [FastGate reference](/api-reference/fastgate/overview) lists where each one differs from a validator, and the V2 methods each have a page with parameters, response fields, and examples.
...
- [Run the quickstart](/products/fastgate-account-api/quickstart)
