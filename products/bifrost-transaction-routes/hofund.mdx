...
Hofund also accepts a signed raw transaction over QUIC, without going through the Bifrost HTTP edge or a copied route URL. The standard lane listens on `7000/UDP` and the protected lane on `9000/UDP`.

QUIC access is issued per customer: contact ThorNode to have your mTLS certificate and its public-key (SPKI) fingerprint onboarded before you connect.
...
Every Hofund surface acknowledges that it took your transaction, not that the transaction landed. A QUIC or WebSocket write is weaker still: it confirms the bytes left your process. Track the signature through Solana RPC until it reaches your commitment or its blockhash expires.
