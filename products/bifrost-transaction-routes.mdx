...
description: "Call bloXroute, Astralane, BlockRazor, and LunarLander transaction routes through one ThorNode Bifrost key."
---

Bifrost puts several Solana transaction providers behind one ThorNode credential. In the dashboard it is the **Bifrost** tab under Endpoints; ThorNode's own sender, Hofund, has its own tab and uses the same key. Select a pass or rental, generate a Bifrost key, then copy the complete route URL for the provider operation you want to call.
...
| Item                 | Value                                                                             |
| -------------------- | --------------------------------------------------------------------------------- |
| Dashboard path       | **Endpoints → Bifrost**                                                        |
| Access               | An eligible pass or rental with at least one Bifrost location                     |
| Bifrost key       | The dashboard generates one per access and reuses it in each enabled location URL |
| Route URL            | `https://<location-base>/<bifrost-key>/<route>`, copied from the dashboard     |
| Authentication       | The Bifrost key sits in the URL path; do not add a provider API key            |
...
<Steps>
  <Step title="Select access and location">
    Open **Endpoints → Bifrost**, select the pass or rental your application
  ...
  <Step title="Choose a provider operation">
    ...
    complete route URL with your Bifrost key.
  </Step>
...
<Warning>
  A copied Bifrost route URL contains your Bifrost key. Keep the complete URL
...
<CardGroup cols={2}>
  <Card
    ...
    icon="route"
    href="/api-reference/bifrost/overview"
  >
  ...
  >
    ThorNode's own transaction sender: fast, direct-TPU, and SWQoS lanes with a Thor tip.
  </Card>
  <Card
